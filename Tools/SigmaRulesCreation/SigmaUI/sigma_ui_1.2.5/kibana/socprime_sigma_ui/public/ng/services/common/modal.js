...
require('ui/modules').get('app/socprime_sigma_ui', []).service('modal', ['$compile', function ($compile) {
    return {
        show: function ($scope, options) {
            ...
            self.$modal.find('.modal-content').append(prepareElement(
                $scope, $compile,
                '<div class="modal-header"><h5 class="modal-title">' + options.title + '</h5><button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button></div><div class="modal-body' + (options.hideBody == true ? ' hidden' : '') + '">' + options.body + '</div><div class="modal-footer"></div>'
            ));
